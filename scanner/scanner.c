...
/* caplet: enable net.probe, wait 'seconds', show table, disable, and quit */
static char *write_temp_caplet(const char *iface, int seconds) {
    ...
    /* caplet: net.probe on, wait seconds, show table, net.probe off, quit */
    fprintf(f,
...
static int run_bettercap_and_collect(const char *iface, const char *caplet_path, char ips[][INET_ADDRSTRLEN], int *count) {
    ...
    if (WIFEXITED(status)) {
        ...
        fprintf(stderr, "bettercap killed by signal %d\n", WTERMSIG(status));
    }
    return 0;
}
...
static int run_nmap_from_hosts_with_os(const char *hosts_path) {
    ...
    if (WIFEXITED(status)) {
        int code = WEXITSTATUS(status);
        if (code != 0) fprintf(stderr, "nmap exited with code %d\n", code);
    } else if (WIFSIGNALED(status)) {
        fprintf(stderr, "nmap killed by signal %d\n", WTERMSIG(status));
    }
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (do_nmap && ip_count > 0) {
        fprintf(stderr, "Running nmap -O -iL hosts.txt for OS detection (root recommended)...\n");
        if (run_nmap_from_hosts_with_os("hosts.txt") != 0) {
            fprintf(stderr, "Error running nmap\n");
            return 1;
        }
    } else if (do_nmap) {
